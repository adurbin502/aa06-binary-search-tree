...
describe("Binary Tree Practice", () => {
  ...
  beforeEach(function () {
    ...
    bstRootBig.right.right.right.right.left = new TreeNode(
      14
    );
    ...
    bstRootUnbalanced.right.right.right.right =
      new TreeNode(5);
    bstRootUnbalanced.right.right.right.right.right =
      new TreeNode(6);
    bstRootUnbalanced.right.right.right.right.right.right =
      new TreeNode(7);
  });
  ...
  beforeEach(function () {
    ...
    btRootBig.right.right.right.right.left = new TreeNode(
      1
    );
    ...
    btRootUnbalanced.right.right.right.right = new TreeNode(
      7
    );
    btRootUnbalanced.right.right.right.right.right =
      new TreeNode(6);
    btRootUnbalanced.right.right.right.right.right.right =
      new TreeNode(5);
  });
  ...
  describe("findMinBST", () => {
    ...
    it("can be called on a subtree", function () {
      ...
      expect(
        findMinBST(bstRootUnbalanced.right.right)
      ).to.equal(3);
  ...
  describe("findMaxBST", () => {
    ...
    it("can be called on a subtree", function () {
      ...
      expect(
        findMaxBST(bstRootUnbalanced.right.right)
      ).to.equal(7);
  ...
  describe("findMinBT", () => {
    ...
    it("can be called on a subtree", function () {
      ...
      expect(
        findMinBT(btRootUnbalanced.right.right.right.right)
      ).to.equal(5);
  ...
  describe("getHeight", () => {
    ...
    it("should work on a subtree", function () {
      ...
      expect(getHeight(bstRootUnbalanced.right)).to.equal(
        5
      );
  ...
  describe("countNodes", () => {
    ...
    it("should work on a subtree", function () {
      ...
      expect(countNodes(bstRootUnbalanced.right)).to.equal(
        6
      );
      ...
      expect(
        countNodes(btRootUnbalanced.right.right)
      ).to.equal(5);
  ...
  describe("balancedTree", () => {
    ...
    it("should work when starting on a subtree", function () {
      ...
      expect(balancedTree(bstRootUnbalanced.right)).to.be
        .false;
    ...
    it("should compare the heights of every node in the tree", function () {
      ...
      expect(balancedTree(bstRootBig.right.right)).to.be
        .false;
      expect(balancedTree(bstRootBig.right.right.right)).to
        .be.false;
      expect(
        balancedTree(bstRootBig.right.right.right.right)
      ).to.be.true;
      ...
      expect(balancedTree(btRootBig.right.right)).to.be
        .false;
      expect(balancedTree(btRootBig.right.right.right)).to
        .be.false;
      expect(
        balancedTree(btRootBig.right.right.right.right)
      ).to.be.true;
    });
  });
  ...
  describe("getParentNode", () => {
    ...
    it("returns null if the target is the root", function () {
      ...
      expect(getParentNode(bstRootUnbalanced, 1)).to.equal(
        null
      );
      expect(getParentNode(bstRootBig, 8)).to.equal(null);

      expect(getParentNode(btRoot, 1)).to.equal(null);
      expect(getParentNode(btRootUnbalanced, 4)).to.equal(
        null
      );
    ...
    it("returns undefined if the target cannot be found", function () {
      ...
      expect(getParentNode(bstRootUnbalanced, 9)).to.equal(
        undefined
      );
      expect(getParentNode(bstRootBig, 9)).to.equal(
        undefined
      );

      expect(getParentNode(btRoot, 9)).to.equal(undefined);
      expect(getParentNode(btRootUnbalanced, 9)).to.equal(
        undefined
      );
      expect(getParentNode(btRootBig, 14)).to.equal(
        undefined
      );
    });
  });
  ...
  describe("inOrderPredecessor", () => {
    it("returns the in-order predecessor of the target node", function () {
      ...
      expect(
        inOrderPredecessor(bstRootUnbalanced, 6)
      ).to.equal(5);
      expect(inOrderPredecessor(bstRootBig, 5)).to.equal(4);
      expect(inOrderPredecessor(bstRootBig, 10)).to.equal(
        8
      );
    });
    ...
    it("returns null if starting node is the first in-order node", function () {
      ...
      expect(
        inOrderPredecessor(bstRootUnbalanced, 1)
      ).to.equal(null);
      expect(inOrderPredecessor(bstRootBig, 1)).to.equal(
        null
      );
    });
  });
  ...
  describe("deleteNodeBST", () => {
    ...
    it("correctly deletes a node with one child", function () {
      ...
      expect(
        bstRootUnbalanced.right.right.right.val
      ).to.equal(5);
      expect(
        bstRootUnbalanced.right.right.right.right.val
      ).to.equal(6);
      expect(
        bstRootUnbalanced.right.right.right.right.right.val
      ).to.equal(7);

      deleteNodeBST(bstRootBig, 15);
      expect(
        bstRootBig.right.right.right.right.val
      ).to.equal(14);
    });
    ...
    it("deletes a node with two children, replacing it with the in-order predecessor or successor", function () {
      ...
      expect(bstRootBig.left.right.val).to.satisfy(
        function (num) {
          return num === 4 || num === 6;
        }
      );
